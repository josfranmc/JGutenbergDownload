...
When you build the project with Maven you get two jars in the target directory: _JGutenbergDownload-2.0.2.jar_ and _JGutenbergDownload-2.0.2-shaded.jar_. The first one is the standard jar of the project. The second one is an _uber_ jar with all necessary dependencies, which is suitable to use from command line.    
...
It is possible to run the program from the command line. To this purpose, you may use the _JGutenbergDownload-2.0.2-shaded.jar_ package with any of the options above:

~~~
java -jar JGutenbergDownload-2.0.2-shaded.jar -l en -s mybooks -m 20
~~~
